...
import { getPostThumbnail, getResizedImage } from '../../utils/ImageApis';
...
const CommentItemHivemind = (props: Props) => {
  const {
    navigation,
    isBlog,
    route,
  ...
  const thumbnail = getResizedImage(statePost.json_metadata.image?.[0]);
  ...
  const PromotionBlog = () => {
    return (
      <TouchableOpacity
        ...
        style={{
          opacity: statePost?.stats.hide ? 0.3 : 1,
  ...
  const BlogsCard = () => {
    return (
      <TouchableOpacity
        onPress={handlePostClick}
        style={{ opacity: statePost?.stats.hide ? 0.3 : 1 }}>
